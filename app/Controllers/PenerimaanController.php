...
class PenerimaanController extends BaseController
{
    public function dataPenerimaanMedis()
    {
        ...
        if (session()->has('jwt_token')) {
            ...
            if ($response_penerimaan && $response_pengajuan) {
                ...
                // Decode responses
                $penerimaan_medis_data = json_decode($response_penerimaan, true);
                $pengajuan_data = json_decode($response_pengajuan, true);

                // Check if both responses are successful
                if ($http_status_code_penerimaan === 200 && $http_status_code_pengajuan === 200) {
                    // Return view with both penerimaan and pengajuan data
                    return view('/admin/pengadaan/medis/penerimaan/data_penerimaan', [
                        'penerimaan_medis_data' => $penerimaan_medis_data['data']['penerimaan_barang_medis'],
                        'pengajuan_data' => $pengajuan_data['data'],
                        'meta_data' => $penerimaan_medis_data['data'],
                        'title' => $title
                    ]);
                } else {
                    // Error: Failed to fetch data
                    return "Error fetching data. HTTP Status Code Penerimaan: $http_status_code_penerimaan, HTTP Status Code Pengajuan: $http_status_code_pengajuan";
                }
            } else {
    ...
    public function tambahPenerimaanMedis()
    {
        ...
        //     'pemesanan' => '/pengadaan/pemesanan',
        //     'pegawai' => '/pegawai',
        //     'barang_medis' => '/inventaris/medis',
        // ];

        // // Initialize cURL requests
        // $ch = [];
        // foreach ($api_urls as $key => $url) {
        //     $ch[$key] = curl_init($this->api_url . $url);
        ...
        // $responses = [];
        // foreach ($ch as $key => $request) {
        //     $responses[$key] = curl_exec($request);
        //     if (!$responses[$key]) {
        //         return "Error fetching data from " . $api_urls[$key];
        //     }
        // }
        ...
        //     $http_status_code = curl_getinfo($ch[$key], CURLINFO_HTTP_CODE);
        //     if ($http_status_code !== 200) {
        //         $errors[] = "Error: HTTP status code " . $response . " received from " . $api_urls[$key];
        //     }
        //     $data[$key] = json_decode($response, true);
        // }

        // if (!empty($errors)) {
        //     // If there are errors, return them all concatenated as a single string
        //     return implode("<br><br>", $errors);
        // }
        // // Close cURL requests
        // foreach ($ch as $request) {
        //     curl_close($request);
        // }

        // // Render view with retrieved data
        // return view('/admin/pengadaan/medis/penerimaan/tambah_penerimaan', [
        //     'pegawai_data' => $data['pegawai']['data'],
        //     'pengajuan_data' => $data['pengajuan']['data'],
        //     'pemesanan_data' => $data['pemesanan']['data'],
        //     'barang_medis_data' => $data['barang_medis']['data'],
        //     'token' => $token,
        //     'title' => $title
        // ]);
