...
class TagihanController extends BaseController
{
        ...
        if (session()->has('jwt_token')) {
            ...
            if ($response_tagihan) {
                if ($http_status_code_tagihan === 200) {
                    ...
                    return "Response tagihan data:" . $response_tagihan;
                }
            } else {
    ...
    public function editTagihanMedis($tagihanId)
    {
        if (session()->has('jwt_token')) {
            ...
            if ($response_tagihan && $response_pesanan && $response_pengajuan) {
                $http_status_code_tagihan = curl_getinfo($ch_tagihan, CURLINFO_HTTP_CODE);
                $http_status_code_pengajuan = curl_getinfo($ch_pengajuan, CURLINFO_HTTP_CODE);
                ...
                if ($http_status_code_tagihan === 200 && $http_status_code_pesanan === 200 && $http_status_code_pengajuan === 200) {
                    ...
                    return "Error fetching data. HTTP Status Code tagihan: $http_status_code_tagihan, HTTP Status Code Pesanan: $http_status_code_pesanan";
                }
            } else {
